...
<sub>4パネルの連携動作：scope ツリー、セキュリティバッジ付き MCP サーバー一覧、詳細 inspector、セキュリティ検出結果。検出項目クリックで該当サーバーに直接遷移</sub>

**単体スキャナーとの違い：** CCO は検出したら、そのままクリックで scope ツリー内の MCP サーバーに飛べる。削除も移動も設定確認も、ツールを切り替えずにその場で完結。
...
|---|:---:|:---:|:---:|:---:|
| scope 階層（Global > Workspace > Project） | **対応** | 非対応 | 非対応 | 一部 |
...
Global                    ← マシン上の全セッションにロード
  └─ Workspace            ← このフォルダ配下のプロジェクト全部にロード
...
**CCO なら完全な scope ツリーが見える。** どの memory、skill、MCP サーバーがどのプロジェクトに影響してるか、一覧で確認。正しい scope に drag するだけ。
...
3. **3パネル dashboard を描画** — scope ツリー、カテゴリアイテム、内容プレビュー付き詳細パネル
