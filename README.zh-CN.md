...
**扫描** → 把所有 memory、skill、MCP server、rule、command、agent、hook、plugin、plan、session 全部展示出来。所有 scope，一棵树。
...
<sub>四个面板联动：scope 树、带安全标记的 MCP server 列表、详情查看器、安全扫描结果——点任意一条就能直接跳到对应 server</sub>

**跟独立安全扫描器有啥区别？** CCO 扫出来的问题，你直接点就能跳到 scope 树里那个 MCP server 条目。删、移、查配置——不用切工具。
...
|---|:---:|:---:|:---:|:---:|
| Scope 层级（Global > Workspace > Project） | **有** | 没有 | 没有 | 部分 |
...
**CCO 把完整的 scope 树展示给你。** 哪些 memory、skill、MCP server 影响了哪些项目，一目了然——然后一拖就能放到对的 scope。
...
3. **渲染三栏面板**——scope 树、分类条目、详情面板（带内容预览）
